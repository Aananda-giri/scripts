...
concise, source-grounded answer via Gemini.
...
```
CSV → HTML clean → Section detection → Chunk → Gemini embed → Qdrant upsert
...
2. **Gemini API key has sufficient quota.** Embedding 1,000 jobs (~6 chunks each) requires ~6,000 API calls. The free tier (1,500 RPM) handles this with batching.
